...
const FilePathList = ({
  ...
  onClick,
  width = '400px',
  listHeight: height = '500px',
  selectedIndex,
}: FilePathListProps) => {
  log('Render on FilePathList')
  const filePathRows = filePathList.map(
    (filepath: FilePathWrapper, index: number) => (
      <FileRow
        key={index}
        index={index}
        filePathWrapper={filepath}
        onClicked={() => onClick(filepath, index)}
  ...
  return (
    <Paper style={{ padding: '10px', width: width }}>
      ...
      <div style={{ overflowY: 'scroll', height: height }}>{filePathRows}</div>
    </Paper>
  )
}
